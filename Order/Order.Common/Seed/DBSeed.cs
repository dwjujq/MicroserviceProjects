    ...
    public class DBSeed
    {
        ...
        public static async Task SeedAsync(MyContext myContext, string WebRootPath)
        {
            try
            {
                ...
                Console.WriteLine("************ Order DataBase Set *****************");
                ...
                var referencedAssemblies = System.IO.Directory.GetFiles(path, "Order.Model.dll")
                   .Select(Assembly.LoadFrom).ToArray();
                var modelTypes = referencedAssemblies
                   .SelectMany(a => a.DefinedTypes)
                   .Select(type => type.AsType())
                   .Where(x => x.IsClass && x.Namespace is "Order.Model.Models")
            ...
            {
                throw new Exception(
                    $"1、若是Mysql,查看常见问题:https://github.com/anjoy8/Order/issues/148#issue-776281770 \n" +
                    $"2、若是Oracle,查看常见问题:https://github.com/anjoy8/Order/issues/148#issuecomment-752340231 \n" +
                    "3、其他错误：" + ex.Message);
            }
        }
        ...
        public static void MigrationLogs(MyContext myContext)
        {
            ...
            var modelTypes = referencedAssemblies
               .SelectMany(a => a.DefinedTypes)
               .Select(type => type.AsType())
               .Where(x => x.IsClass && x.Namespace != null && x.Namespace.StartsWith("Order.Model.Logs"))
               .ToList();

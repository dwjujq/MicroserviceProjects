...
public class DbTest(ITestOutputHelper testOutputHelper)
{
    ...
    public void Test_CreateDataBase()
    {
        ...
        string connectionString2 = "Database=Order;TrustServerCertificate=true;Persist Security Info=False;Trusted_Connection=True;server=(local)";

        // 创建数据库
        using (SqlConnection masterConnection = new SqlConnection(connectionString))
        {
            masterConnection.Open();
            string createDbQuery = $"CREATE DATABASE [Order];";

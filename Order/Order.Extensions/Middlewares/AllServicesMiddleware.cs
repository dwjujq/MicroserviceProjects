...
namespace Order.Extensions.Middlewares
{
    /// <summary>
    /// 查看所有注入的服务
    /// </summary>
    public static class AllServicesMiddleware
    {
        public static void UseAllServicesMiddle(this IApplicationBuilder app, IServiceCollection _services)
        {
            ...
            //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "Order.Services.dll")).GetTypes().ToList());
            //tsDIAutofac.AddRange(Assembly.LoadFrom(Path.Combine(AppContext.BaseDirectory, "Order.Repository.dll")).GetTypes().ToList());
